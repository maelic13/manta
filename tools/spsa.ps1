...
.DESCRIPTION
    ...
    option (docs/UCI.md §7). Manta's registry is staged by phase; every knob a
    group names must already be advertised, or the tune moves nothing. This
    script verifies that against the actual binary before spending a single
    game.

    Other prerequisites:
      - ./tools/setup_tools.ps1 once if tools\bin\fastchess.exe or
        tools\weather-factory\main.py is missing (this script also auto-runs it
        if weather-factory is absent).
      - A binary in tools\test_engines (see ./tools/build_test.ps1).
...
.PARAMETER StopAfter
    Absolute iteration at which this invocation stops. Default 0 means the
    full planned horizon. This does not alter the schedule; use it for a
    mandatory checkpoint or a shorter work session, then resume the same state.
...
.PARAMETER REnd
    ...
    simulation validated against a real Rarog trajectory; fishtest's own
    default is 0.002, the same order. Larger = hotter = more late wander.
...
.PARAMETER EngineSuffix
    Suffix of the binary in tools\test_engines. Accepts a bare suffix
    (manta-<s>.exe) or a full "*.exe".
...
.PARAMETER LaunchOnly
    ...
    and Resume are ignored. Iterations must remain the original planned horizon;
    StopAfter is the session stop target.

.PARAMETER ShowValues
    Print the current tuned values from tuner\state.json and exit.
...
.EXAMPLE
    ...
    ./tools/build_test.ps1 -Suffix tune
    ./tools/spsa.ps1 -ConfigGroup smoke -EngineSuffix tune -Iterations 2500

.EXAMPLE
    # Continue an interrupted run:
    ./tools/spsa.ps1 -ConfigGroup smoke -Resume

.EXAMPLE
    # Set up now, launch later:
    ./tools/spsa.ps1 -ConfigGroup smoke -SetupOnly
    ./tools/spsa.ps1 -ConfigGroup smoke -LaunchOnly
#>
param(
    [string]$ConfigGroup = "smoke",
    [int]$Iterations = 5000,
    [int]$StopAfter = 0,
    [double]$REnd = 0.0031,
...
# weather-factory auto-detects the book format from the extension
# (cutechess.py: format={book.split('.')[-1]}), so the EPD works unmodified —
# and keeps SPSA on the same book as sprt.ps1.
...
if ($ShowValues) {
    $statePath = Join-Path $wfRoot "tuner\state.json"
    ...
    $ccPath = Join-Path $wfRoot "cutechess.json"
    ...
    foreach ($p in $state.uci_params) {
        "{0,-24} = {1}" -f $p.name, [int][Math]::Round([double]$p.value) | Write-Host
    }
    return
}
...
if (-not $LaunchOnly) {
    if ($EngineSuffix -eq "") {
        throw "-EngineSuffix is required for setup (the binary in tools\test_engines to tune)."
    }
    $engineFile = if ($EngineSuffix.EndsWith(".exe")) { $EngineSuffix } else { "manta-$EngineSuffix.exe" }
    ...
    # ── The check that makes an SPSA on Manta meaningful ─────────────────
    # weather-factory sends every tuned knob as `option.<Name>=<value>`. Manta
    # answers an unadvertised option with `info string unknown option ...
    # ignored` and plays on, so a tune over knobs the binary does not expose
    # burns days of games measuring pure noise and "converges" on nothing.
    # Refuse before the first game instead.
    ...
    $tunedNames = @((Get-Content $srcConfig -Raw | ConvertFrom-Json).PSObject.Properties.Name)
    ...
    if ($missing.Count -gt 0) {
        throw ("$engineFile does not advertise: $($missing -join ', ')`n" +
               "  Advertised: $($advertised -join ', ')`n" +
               "SPSA can only tune UCI options the engine exposes. Add them to the option " +
               "registry (docs/UCI.md §7) and rebuild, or edit $srcConfig.")
    }
    $sourceConfig = Get-Content $srcConfig -Raw | ConvertFrom-Json
    foreach ($parameter in $sourceConfig.PSObject.Properties) {
        $declaration = $advertisedDetails | Where-Object { (& $normalize $_.Name) -eq (& $normalize $parameter.Name) } | Select-Object -First 1
        if ($declaration.Type -ne 'spin') {
            throw "$($parameter.Name) is advertised as '$($declaration.Type)', not a tunable spin option."
        }
        $configValue = [int64]$parameter.Value.value
        $configMin = [int64]$parameter.Value.min_value
        $configMax = [int64]$parameter.Value.max_value
        $configStep = [double]$parameter.Value.step
        if ($configValue -ne [int64]$declaration.Default -or
            $configMin -lt $declaration.Min -or $configMax -gt $declaration.Max -or
            $configMin -ge $configMax -or $configValue -lt $configMin -or
            $configValue -gt $configMax -or $configStep -le 0) {
            throw ("Invalid SPSA declaration for $($parameter.Name): config value=$configValue " +
                "range=[$configMin,$configMax] step=$configStep; engine default=$($declaration.Default) " +
                "range=[$($declaration.Min),$($declaration.Max)].")
        }
        $endPerturbation = $configStep / [Math]::Pow($Iterations, 0.102)
    ...
    Write-Host "Tunable options verified against $engineFile : $($tunedNames -join ', ')" -ForegroundColor Green
    ...
    if ($wfCuteContent -notmatch 'MANTA_AFFINITY_PATCH_V2' -or
        $wfCuteContent -notmatch [regex]::Escape("-use-affinity $expectedAffinityCpus ")) {
        throw "weather-factory is not carrying the verified Manta affinity patch; run tools/setup_tools.ps1."
    }
    ...
    if ((Get-Content $wfSpsaPy -Raw) -notmatch 'MANTA_SCHEDULE_FIX_V1') {
        throw "weather-factory is not carrying the SPSA schedule fix (decay per-iteration); run tools/setup_tools.ps1."
    }
    if ((Get-Content $wfSpsaPy -Raw) -notmatch 'MANTA_TRANSACTIONAL_STEP_V2') {
        throw "weather-factory is not carrying the transactional resume fix; run tools/setup_tools.ps1."
    }
    if ($wfCuteContent -notmatch 'MANTA_FIXED_OPTIONS_V1') {
        throw "weather-factory cannot apply fixed architecture options; run tools/setup_tools.ps1."
    }
    if ($wfCuteContent -notmatch 'MANTA_ADJUDICATION_PATCH_V4' -or
        $wfCuteContent -match '(?m)^\s*"-(resign|draw)') {
        throw ("weather-factory is still passing adjudication; new SPSA runs require natural termination. " +
            "Run tools/setup_tools.ps1.")
    }
    # Without this, an option name containing a space (Manta's registry uses
    # several) is split across two argv entries and never reaches the engine.
    if ($wfCuteContent -notmatch 'MANTA_OPTION_QUOTING_V1' -or
        (Get-Content $wfSpsaPy -Raw) -notmatch 'MANTA_OPTION_QUOTING_V1') {
        throw "weather-factory is not carrying the option-name quoting patch (spaced UCI names); run tools/setup_tools.ps1."
    }
    if ($wfCuteContent -notmatch 'MANTA_TIME_MARGIN_PATCH_V1' -or
        $wfCuteContent -notmatch 'MANTA_FAIL_CLOSED_MATCH_V1' -or
        $wfCuteContent -notmatch 'MANTA_TIME_LOSS_POLICY_V1' -or
        $wfCuteContent -match '"-recover "') {
        throw "weather-factory lacks the verified margin/fault/time-loss policy; run tools/setup_tools.ps1."
    }
    ...
    # Options held FIXED at a chosen value for both perturbed engines. They are
    # deliberately absent from config.json so they cannot absorb another
    # parameter's noisy gradient.
    ...
    if (Test-Path $fixedConfigPath) {
        ...
        foreach ($option in $fixedConfig.PSObject.Properties) {
            if ($advertisedNormalized -notcontains (& $normalize $option.Name)) {
                throw "$engineFile does not advertise fixed option '$($option.Name)' from $fixedConfigPath."
            }
            $fixedOptions[$option.Name] = [int]$option.Value
        }
    }

    $scoreTimeLosses = $ConfigGroup -eq "time_sensitivity"
    $cutechessJson = @{
        engine        = $engineName
        ...
        time_margin   = 20
        score_time_losses = $scoreTimeLosses
    } | ConvertTo-Json
    ...
    # Because both constants are back-solved from the horizon, changing the
    # planned iteration count can never silently change the END behaviour, and
    # `a` can never be left stale when the schedule shape changes.
    #
    # ⚠ ONLY THE `a` HALF IS IMPLEMENTED. weather-factory keeps `a`/`c` global
    # and puts the per-parameter scale in each config's `step`. `c` stays 1.0
    # and is NOT back-solved, and no knob declares a `c_end` — so a config's
    # `step` is the perturbation at iteration 1, NOT at the horizon:
    ...
    # here. Only the NAME misleads — reading `step` as `c_end` gives the wrong
    # answer about whether a knob survives to the horizon.
    #     a = r_end * (A + N)^alpha / N^(2*gamma)
    # Practical consequence: the engine receives round(value), so an integer
    # knob needs step * c_t(N) >= 0.5, i.e. step >= 2. A step-1 integer knob
    # goes dead at it > 2^(1/gamma) ~= 894.
    $alpha = 0.601
    $gamma = 0.102
    # ⚠⚠ DO NOT name these `$A` and `$a`. PowerShell variable names are
    # CASE-INSENSITIVE, so `$A` and `$a` are ONE variable: the gain assignment
    # silently overwrites the damping term and spsa.json ships `"A": 0.0965`
    # where it needs `"A": 500` — A ≈ 0, i.e. NO damping over the first 10% of
    # the run. This bit Rarog for real; the assertions below are what make it
    # un-shippable again.
    $dampingA = [int]([Math]::Floor($Iterations / 10))
    if ($dampingA -le 0) { throw "-Iterations $Iterations is too small: damping A would be 0 (no damping)." }
    ...
    # Index with brackets — property access on the hashtable would be
    # case-insensitive and silently return the wrong one of the two.
    ...
    # on mutable paths or the operator's memory. The engine sidecar holds its
    # build flavour and bench; this manifest binds that binary to this exact
    # SPSA configuration, runner and opening source.
    ...
    if ($runManifest.repo_dirty) {
        Write-Warning "SPSA setup was made from a dirty repository. Commit the prepared workflow and rebuild before a long launch."
    }
    Write-Host ""
...
if ($launchCuteContent -notmatch 'MANTA_AFFINITY_PATCH_V2' -or
...
if ((Get-Content $launchSpsaPy -Raw) -notmatch 'MANTA_SCHEDULE_FIX_V1') {
    throw "weather-factory is not carrying the SPSA schedule fix (decay per-iteration); run tools/setup_tools.ps1."
}
if ((Get-Content $launchSpsaPy -Raw) -notmatch 'MANTA_TRANSACTIONAL_STEP_V2') {
    throw "weather-factory is not carrying the transactional resume fix; run tools/setup_tools.ps1."
}
if ($launchCuteContent -notmatch 'MANTA_FIXED_OPTIONS_V1') {
    throw "weather-factory cannot apply fixed architecture options; run tools/setup_tools.ps1."
}
if ($launchCuteContent -notmatch 'MANTA_OPTION_QUOTING_V1' -or
    (Get-Content $launchSpsaPy -Raw) -notmatch 'MANTA_OPTION_QUOTING_V1') {
    throw "weather-factory is not carrying the option-name quoting patch (spaced UCI names); run tools/setup_tools.ps1."
}
if ($launchCuteContent -notmatch 'MANTA_TIME_MARGIN_PATCH_V1' -or
    $launchCuteContent -notmatch 'MANTA_FAIL_CLOSED_MATCH_V1' -or
    $launchCuteContent -notmatch 'MANTA_TIME_LOSS_POLICY_V1' -or
    $launchCuteContent -match '"-recover "') {
    throw "weather-factory lacks the verified margin/fault/time-loss policy; run tools/setup_tools.ps1."
}
# The no-adjudication rule is required to START a tune, but NEVER blocks a
# RESUME. A run that began under an old rule must finish under it: switching
...
if (-not (Test-Path (Join-Path $wfRoot "tuner\state.json")) -and
    ($launchCuteContent -notmatch 'MANTA_ADJUDICATION_PATCH_V4' -or
     $launchCuteContent -match '(?m)^\s*"-(resign|draw)')) {
    throw ("weather-factory is still passing adjudication; run tools/setup_tools.ps1 before starting " +
        "a new tune.")
}
...
if ([int]$launchConfig.time_margin -ne 20) {
    throw "cutechess.json time margin is $($launchConfig.time_margin) ms, expected the qualified 20 ms."
}
$expectedScoreTimeLosses = $ConfigGroup -eq "time_sensitivity"
if ([bool]$launchConfig.score_time_losses -ne $expectedScoreTimeLosses) {
    throw "cutechess.json score_time_losses must be $expectedScoreTimeLosses for group $ConfigGroup."
}

$launchManifestPath = Join-Path $wfRoot "tuner\run_manifest.json"
if (-not (Test-Path $launchManifestPath)) {
    throw "Missing run_manifest.json; run setup again before launching this tune."
}
...
if ([bool]$launchManifest['score_time_losses'] -ne $expectedScoreTimeLosses) {
    throw "run manifest time-loss policy does not match group $ConfigGroup."
}

# ─── Multi-session bookkeeping ────────────────────────────────────────────
# Long tunes ALWAYS span several sessions. Three things make that safe, and
# each was broken in upstream weather-factory:
#   1. the log must APPEND on resume (it truncated — a Rarog tune lost 1,086 of
#      its 3,670 iterations, and the trajectory is what the bake filter reads);
...
#      state.json, so re-passing -Iterations on a resume silently does nothing.
#      Get it right the first time or archive and restart.
...
if ($doneIters -gt 0) {
    ...
    Write-Host ("  Progress: iteration $doneIters / $Iterations " +
        "($([Math]::Round(100.0 * $doneIters / $Iterations, 1))% of horizon) - $remain to this stop.") -ForegroundColor Cyan
} else {
