...
| `FILE-002` | A reproducible job or release that consumes external data shall record its role, byte size and cryptographic content hash in a compact committed or archived manifest. | 4 onward | Manifest validation |
...
| `PERF-007` | Timing evidence shall use an idle host, warm-up, repeated samples and recorded CPU/OS/power/topology/toolchain data. Concurrent builds, games or benchmarks invalidate timing evidence. | 2 onward | Manifest validation |
