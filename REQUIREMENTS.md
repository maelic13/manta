...
| `SCORE-010` | Selective search shall represent nominal child horizon, extension and reduction separately before composing searched depth. Node-entry route and completed outcome attribution shall remain distinct from score/bound/provenance authority; an upper bound denotes fail-low, exact denotes exact and lower denotes cutoff. Context infrastructure alone shall not affect legality, terminal/draw handling, TT/PV authority or playing policy. Diagnostic attribution shall charge every visited node to the innermost speculative context on its path, so the charges partition the visited tree exactly, and separately to each enclosing context; transposition lookups and stores shall be partitioned exactly by outcome. Any search that re-enters a node at its own ply shall save and restore the observer's per-ply path facts, so a probe cannot be billed the work of the node that launched it. | 5 | Context on/off equivalence, depth properties, exact charge/lookup/store partitions, same-ply restoration property and observation accounting |
| `SCORE-011` | Production depth authority shall extend legal check evasions without changing nominal completed-root depth, reduce only eligible non-root searches whose missing move evidence warrants lower confidence, and derive singular extensions only from legal ordinary TT evidence after a same-position exclusion search fails low. The default-off Step-6.5.3 MAN-S31 candidate may suppress only non-root blanket check increments; checked-root extension, legal evasions including in-check qsearch, terminal/draw precedence and mate-distance semantics remain fixed. The default-off Step-6.5.5 MAN-S33 candidate may shorten only the singular exclusion horizon while preserving the legal ordinary TT producer, exact excluded move, threshold, null disablement and fail-low consumer. Exclusion searches shall not manufacture terminal, TT, PV, history, null-move or reusable score authority. The complete family and each producer shall remain compile-time ablatable. | 5, 6.5.3, 6.5.5 | Focused check/IIR/singular/exclusion and candidate both-arm legality/terminal/restoration tests, production/family-off fingerprints, observation accounting and registered 1T games |
...
| `SCORE-033` | The accepted Step-6.5.10.1 `MAN-S35` mate-window policy shall, at every non-root main-search node after reached-draw and maximum-ply precedence and before the table probe, intersect the requested window with the band the rules of chess still allow at that ply, `[matedIn(ply), mateIn(ply + 1)]`. An empty intersection shall return the proven bound it already held; otherwise the clipped window shall be the one the table probe, forward proofs, move loop, PVS test and final bound classification read, and a result at or below its alpha shall be upper, at or above its beta lower, and between them exact. Those verdicts remain valid for a caller holding a wider window because the clipped ranges hold no score the rules can produce with a legal move available. Zero windows shall be clipped exactly as the superseded crossing-only `MAN-S32` test decided them, so the changed behavior is confined to open windows. Terminal, draw and tablebase precedence, `SCORE-004` distance-relative table normalization, root completion, aspiration and root confidence shall remain unchanged, and mate-distance evidence shall keep acquiring no searched-eval, singular or ProbCut authority. The mechanism shall default on and remain compile-time ablatable to the superseded crossing-only tree, which shall keep reconstructing fingerprint `775,451` for archived diagnostics; the production fingerprint was `642,336` until MAN-S36 moved it to `359,259`, and mate windows remain part of that head. `MAN-S32`'s crossing-only arm is removed rather than retained as a third configuration. Its registered 1T gate returned a neutral result, and the mechanism is production by the documented maintainer exception recorded in `EXPERIMENTS.md` rather than by an H1 claim. | 6.5.10 | Band and clip properties from the mate-distance definition, zero-window equivalence, both-side mate distance and legal PV, terminal precedence, table round trip under clipped windows, corpus mate-position identity, feature-off fingerprint, separate ordinary/mate cohort attribution and one registered 1T SPRT |
| `SCORE-034` | The accepted Step-6.5.10 `MAN-S36` coordinated selective-search core shall be one compile-time umbrella, default on in production since its registered 1T gate accepted H1 (670 games, W/L/D `283/80/307`, `+108.68 +/- 17.86` Elo (`+170.97 +/- 26.31` nElo), LLR `2.95` on normalized `[1,5]`, LOS `100%`, draw ratio `34.93%`, pairs ratio `5.41`, pentanomial `[0,34,117,131,53]`, no anomaly of any kind), whose component switches exist for ablation diagnosis only and are never gated separately. Under the core: main, reply and continuation quiet history shall share one bounded linear depth bonus and equal malus applied through the bounded gravity update, rewarding only an exact or cutoff quiet winner and penalising only quiets actually searched before it; the reduction surface shall be one compile-time log-log table in 1024ths of a ply adjusted by node expectation, improving trend, TT-move presence, clamped history and root relief, applied from the third selected move to non-checking quiet non-promotions and losing captures at nodes not in check, never to promotions, good captures or the singular move, converted once at dispatch and clamped to `[0, new_depth - 1]` so a reduced probe keeps one main-search ply; every reduced alpha rise shall be verified at the planned full horizon before it may acquire PV, cutoff, exact or feedback authority, and a zero reduction shall be the ordinary scout; late-move count, quiet futility, history and losing-capture SEE omission shall read one prospective depth derived from that same surface, only at non-PV nodes not in check after one legal move was actually searched, under ordinary windows with non-pawn material; no per-move omission test shall omit a move that gives check, while a late-move count trigger may skip the node's remaining unmade quiets other than direct quiet checks; a non-check quiescence node entered directly from the main search shall additionally generate and search the legal direct quiet checks whose destination is not a losing square, without delta pruning and without generating checks below that first quiescence ply, keeping the accepted terminal witness, stand-pat, tactical SEE/delta and storage rules; a fail-low with omitted siblings shall store as an ordinary upper bound at nominal depth; reverse futility to depth 8 with margins scaled to the evaluator's swings, razoring at depth 1 through quiescence, null move from depth 3 with a depth- and margin-scaled reduction whose same-node real-move verification runs only from depth 10, and internal iterative reduction at every node lacking a legal TT move from depth 4 shall be the node-level proofs; root aspiration shall start from an exact ordinary previous score, widen the failed side geometrically, open that side fully after a bounded number of failures, reset root evidence per attempt and commit only exact attempts. Terminal, draw, mate-distance, tablebase, check and singular extension, ProbCut, quiescence, TT format, publication, clock and thread authority shall remain unchanged, and the umbrella-off tree shall reconstruct the superseded MAN-S35 head at `642,336` in default and observation-enabled builds; production is `359,259`. Seeds are Manta's own and are fitted only in Step 6.5.11. | 6.5.10 | Table monotonicity, clamp and adjustment-sign properties, prospective-depth bound, `lmp_count` monotonicity, aspiration termination, legal PV with check, promotion, first-move and root exemptions, verification-before-authority and zero-reduction scout properties, null-verification scope, quiet-check generation subset and give-check oracle, tactical canaries including WAC.001 at depth 5 in the core arm, both-arm fingerprints, corpus branching and cohort diagnostics, local diagnostic match and one registered 1T SPRT |
...
`775451`. Step 6.5.10.1 is complete and production as `SCORE-033` above; it
changes only the window a non-root node searches and adds no depth, ordering or
feedback consumer, and it moved the production fingerprint to `642336`.
`SCORE-034` is the contract of the Step-6.5.10 coordinated selective-search
core designed in [ADR-0071](docs/adr/0071-coordinated-selective-search-core.md),
and since its registered gate accepted H1 on 2026-09-13 its consumers are
production. Where `SCORE-016`, `SCORE-019`, `SCORE-021` and `SCORE-022`
describe the superseded isolated mechanisms, they remain the contracts of
their archived switches only, which are reachable only with the umbrella off. Any incompatible
...
| `PERF-002` | Performance-sensitive work shall begin from a profile and use controlled A/B measurement on clean production builds. A behavior-identical claim requires exact fingerprint, PV and result identity. A diagnosed legal deterministic mismatch shall receive a causal record and move to the playing-candidate gate rather than being rejected for strength by its fingerprint. | 2 onward | Benchmark manifest and causal ledger |
...
| `PERF-007` | Timing evidence shall use an idle host, warm-up, repeated samples and recorded CPU/OS/power/topology/toolchain data. Development diagnostics may run on the workspace computer, but authoritative games, tuning and final cross-engine comparisons shall run on the separate designated Ryzen 9 5950X with calibrated physical-core placement and no competing timed work. A remote handoff shall bind exact source/archive and binary hashes, compiler/options, features, runner/book inputs and setup-only command; returned manifests, logs, PGNs and checkpoints shall be verified before a verdict is recorded. Recalibrate after a relevant hardware, OS, runner, clock or placement change. | 2 onward | Handoff and run-manifest validation |
...
| `QUAL-018` | Natural game termination | A measured game shall end only by the rules of chess: checkmate, stalemate, the fifty-move rule, threefold repetition or insufficient material. No harness shall apply a resignation threshold, a draw-after-N-moves rule or a move cap, and every run manifest shall record the game-end profile and `adjudication: none`. ADR-0071 owns this decision; verdicts measured under the superseded `strength-v1`/`strength-v2` profiles keep their original recorded conditions and are not comparable with later ones |
...
| Behaviour-neutral hot-path work | Full static/test gates, exact one-thread fingerprint, controlled performance A/B and—during Phase 6.5—one registered remote-host 1T SPRT before production promotion |
...
aspiration and retry authority. Step 6.5.10 adds `SCORE-033` for complete
mate windows and `SCORE-034` for the coordinated selective-search core. The
current total is 117 requirements.
